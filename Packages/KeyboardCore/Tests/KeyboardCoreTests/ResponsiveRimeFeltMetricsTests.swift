...
final class ResponsiveRimeFeltMetricsTests: XCTestCase {
    ...
    func testMarkerLinesAreContentFree() {
        ...
        let publish = ResponsiveRimeFeltMetrics.publishLagMarkerLine(
            lagMs: 40,
            revision: 5,
            pendingAfter: 0,
            coalesced: true
        )
        XCTAssertTrue(publish.contains("PUBLISH lagMs=40"))
        XCTAssertTrue(publish.contains("coalesced=1"))

        let burst = ResponsiveRimeFeltMetrics.burstMarkerLine(count: 4, windowMs: 50)
        XCTAssertTrue(burst.contains("BURST count=4"))
    }
    ...
    func testTrackerAcceptVisiblePublishAndBurst() {
        ...
        var burstSeen = false
        for i in 0..<3 {
            let result = tracker.recordPublish(
                revision: UInt64(i + 1),
                pendingAfter: 0,
                coalesced: i > 0,
