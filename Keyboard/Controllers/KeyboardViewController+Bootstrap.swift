...
extension KeyboardViewController {
    ...
    func bootstrapKeyboard() {
        ...
        controller.onResponsivePresentationNeeded = { [weak self] effects in
            ...
            #if DEBUG
                if self.isHighFidelityDiagnosticsActive {
                    self.diagnosticsJournal.record(
                        code: .rimeOwnerPublished,
                        category: .engine,
                        ...
                        fields: [
                            ...
                            .count(
                                .sessionEpoch,
                                Int(
                                    clamping: self.controller.threadAffineRimeCoordinator?
                                        .diagnostics.sessionEpoch ?? 0
                                )
                            ),
                        ]
                    )
                ...
                if self.isHighFidelityDiagnosticsActive {
                    self.diagnosticsJournal.record(
                        ...
                        fields: [
                            ...
                            .count(
                                .candidateCount,
                                self.controller.state.lastRimeOutput?.candidates.count ?? 0
                            ),
                            .count(
                                .visibleCandidateCellCount,
                                self.candidateCollectionView?.visibleCells.count ?? 0
                            ),
                        ]
                    )
                }
    ...
    extension KeyboardViewController {
        ...
        func p3d1RecordLifecycleMarker(
            ...
            let gate =
                ...
                && controller?.isThreadAffineRimeOwnerEnabled == true
            let revision =
                coordinator?.lastPublished?.revision
