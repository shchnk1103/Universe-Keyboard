...
final class NativeExperienceKeyboardAutomationFeasibilityTests: XCTestCase {
    ...
    /// Controlled INT-003 stimulus. The test runner invokes the product-owned
    /// AX key targets directly so the cadence is not limited by an external
    /// automation RPC. The app's diagnostics, rather than the requested
    /// sleep interval, remain the authority for the observed timing.
    func testTypoCorrectionINT003ControlledAXCadence() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["TYPO_INT003_AX_HARNESS"] == "1",
            "Run only as the authorized INT-003 AX cadence harness."
        )
        continueAfterFailure = false
        ...
        guard preparation.keyboardSurface.isVisible else {
            throw XCTSkip(
                "INT-003 AX harness blocked: Messages did not expose a keyboard surface."
            )
        }
        ...
        guard activation.activated else {
            throw XCTSkip(
                "INT-003 AX harness blocked: Universe Keyboard activation was not proven."
            )
        }

        // This is a fixed synthetic key fixture. It is intentionally not
        // copied into diagnostics or the evidence receipt.
        let keyNames = [
            "w", "i", "m", "e", "n", "j", "i", "n", "t", "i", "a", "n",
            "q", "u", "h", "o", "n", "g", "y", "u", "a", "n",
        ]
        let keys = keyNames.map { keyboardEvidenceKey(named: $0, in: messages) }
        let unavailableKeys = zip(keyNames, keys)
            .filter { !$0.1.waitForExistence(timeout: 5) || !$0.1.isHittable }
            .map(\.0)
        guard unavailableKeys.isEmpty else {
            throw XCTSkip(
                "INT-003 AX harness blocked: unavailable key targets \(unavailableKeys.joined(separator: ", "))."
            )
        }

        let requestedInterKeyDelay = 0.08
        let requestedPostBurstPause = 0.35
        let tapStartedAt = Date()

        for (index, key) in keys.enumerated() {
            key.tap()
            if index < keys.index(before: keys.endIndex) {
                Thread.sleep(forTimeInterval: requestedInterKeyDelay)
            }
        }
        Thread.sleep(forTimeInterval: requestedPostBurstPause)

        let tapFinishedAt = Date()
        let metadata = preparation.metadata
            .merging(activation.metadata) { _, new in new }
            .merging([
                "AX activation": "Universe Keyboard-specific controls were observed before the stimulus.",
                "Tap count": "\(keys.count)",
                "Requested inter-key delay": "0.080 s",
                "Requested post-burst pause": "0.350 s",
                "Tap wall-clock start": iso8601(tapStartedAt),
                "Tap wall-clock finish": iso8601(tapFinishedAt),
                "Content retention": "Fixed key fixture was not written to diagnostics or probe metadata.",
                "Host injection guarantee":
                    "No typeText, pasteboard, setMarkedText, documentContext or host insertion API was used.",
            ]) { _, new in new }

        recordProbe(
            purpose:
                "Exercise the documented cancellable 180 ms contextual-correction pause through real AX key taps.",
            name: "INT-003 controlled AX cadence",
            app: messages,
            failureBoundary:
                "The XCTest-internal AX key sequence completed; debounce, stale-work and UI-application conclusions come from the paired content-free diagnostics receipt.",
            limitationClassification:
                "Requested cadence is harness-controlled, but observed cadence must be computed from app diagnostics; no semantic candidate conclusion is allowed.",
            includeScreenshot: false,
            extraSections: metadata
        )
    }

    /// Test-only coordinate-touch feasibility probe. Key discovery and
    /// Universe Keyboard activation remain AX-based; only the per-key action
    /// changes from XCUIElement.tap() to a pre-resolved center coordinate.
    func testTypoCorrectionINT003ControlledAXCoordinateCadence() throws {
        try XCTSkipUnless(
            ProcessInfo.processInfo.environment["TYPO_INT003_AX_COORDINATE_HARNESS"] == "1",
            "Run only as the authorized INT-003 coordinate-touch feasibility harness."
        )
        continueAfterFailure = false
        ...
        guard preparation.keyboardSurface.isVisible else {
            throw XCTSkip(
                "INT-003 coordinate harness blocked: Messages did not expose a keyboard surface."
            )
        }
        ...
        guard activation.activated else {
            throw XCTSkip(
                "INT-003 coordinate harness blocked: Universe Keyboard activation was not proven."
            )
        }

        // This is a fixed synthetic key fixture. It is intentionally not
        // copied into diagnostics or probe metadata.
        let keyNames = [
            "w", "i", "m", "e", "n", "j", "i", "n", "t", "i", "a", "n",
            "q", "u", "h", "o", "n", "g", "y", "u", "a", "n",
        ]
        ...
        guard unavailableKeys.isEmpty else {
            throw XCTSkip(
                "INT-003 coordinate harness blocked: unavailable key targets \(unavailableKeys.joined(separator: ", "))."
            )
        }

        let coordinates = keys.map {
            $0.coordinate(withNormalizedOffset: CGVector(dx: 0.5, dy: 0.5))
        }
        let requestedInterKeyDelay = 0.08
        let requestedPostBurstPause = 0.35
        let tapStartedAt = Date()
        ...
        for (index, coordinate) in coordinates.enumerated() {
            coordinate.tap()
            if index < coordinates.index(before: coordinates.endIndex) {
                Thread.sleep(forTimeInterval: requestedInterKeyDelay)
            }
        }
        Thread.sleep(forTimeInterval: requestedPostBurstPause)
        ...
        let metadata = preparation.metadata
            .merging(activation.metadata) { _, new in new }
            .merging([
                "AX activation": "Universe Keyboard-specific controls were observed before coordinate dispatch.",
                "Coordinate source": "Center coordinates resolved once from product-owned AX key elements.",
                "Dispatch path": "XCUICoordinate.tap() in one XCTest runner loop.",
                "Tap count": "\(coordinates.count)",
                "Requested inter-key delay": "0.080 s",
                "Requested post-burst pause": "0.350 s",
                "Tap wall-clock start": iso8601(tapStartedAt),
                "Tap wall-clock finish": iso8601(tapFinishedAt),
        ...
        recordProbe(
            purpose:
                "Measure whether pre-resolved XCTest coordinate touches can exercise the documented 180 ms contextual-correction pause.",
            name: "INT-003 coordinate-touch feasibility",
            app: messages,
            failureBoundary:
                "The XCTest coordinate sequence completed; cadence, stale-work and UI-application conclusions come from the paired content-free diagnostics receipt.",
            limitationClassification:
                "Coordinate dispatch is a test-only feasibility method; actual diagnostic timestamps remain authoritative and no semantic candidate conclusion is allowed.",
            includeScreenshot: false,
            extraSections: metadata
        )
    }
