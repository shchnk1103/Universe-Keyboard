...
- [ ] Notification permission is requested only after the user explicitly enables “允许 App 通知” or the shared RIME notification category. New users default off; denied permission turns the App master switch off, preserves category choices, offers a System Settings link and does not disable syncing.
- [ ] The global notification page and RIME sync page show the same effective RIME category state. Closing the last category closes the master switch; disconnecting or reselecting the sync folder does not reset notification or Toast preferences.
- [ ] Allowed RIME notifications cover manual and automatic start/completion/failure or cancellation, accurately identify RIME standard data, Universe settings or both, and reveal no user data. With the App foregrounded, enabled operation Toasts suppress the duplicate banner/sound but keep the Notification Center entry; disabled Toasts allow banner/list/sound.
