...
- [ ] The global notification page and RIME sync page show the same effective RIME category and child-notification states. Closing the last child closes the category and master switch; disconnecting or reselecting the sync folder does not reset notification or Toast preferences. Notification choices never change the actual automatic-sync master or scope switches.
