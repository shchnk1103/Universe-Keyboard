...
**2026-09-08 current increment:** Cross-scheme **CS-03–CS-10 plus CS-F1/CS-F3** is pushed to `codex/scheme-delivery-fix` at `fd8ba55` and GitHub CI is green (PR #100 remains draft): identical-receipt idempotent no-op, inactive-uninstall peer-retain, active-uninstall Luna-only peer coverage, final-scheme Luna fallback, retained-peer redeploy coverage, and named failure injections in `SchemaManagerTests` and `SchemeResourcePreparationCoexistenceTests`. The separately authorized `CSF-PAIR-01/02` addition is pushed at `a6771ed` with GitHub CI green: symmetric Wanxiang F1 and combined manager/real-installer F3 proof in both directions, focused and full App + Keyboard tests, and an independent Pass-with-conditions review. Separately authorized `CS09-10-01` is a local candidate: real installer inventory and retained-peer byte proof in both directions passed focused and full App + Keyboard tests; independent review is pending. CS-F1/CS-F3 and CS-09/10 each received an independent Pass-with-conditions review. Base tip before the matrix work: `944477a`. CS-01/02 already on branch (`2813428`). Human superseded peer-prefer B with Luna-only fallback.
...
|---|---|
| Lifecycle | Active |
| Current Phase | Cross-scheme matrix: **CS-01/02 done** (committed `2813428`); **CS-03/04 done locally** (identical no-op + peer retain / Wanxiang identity-change upgrade path; eng `cb06a08`); **CS-05/06 done locally** (inactive uninstall retains active peer and does not force Luna); **CS-07/08 done locally** (active uninstall always deploys Luna before target removal); **CS-F1/CS-F3 pushed candidate, independently reviewed with conditions**; **CSF-PAIR-01/02 pushed at `a6771ed`, CI green, independently reviewed with conditions** (symmetric Wanxiang F1 plus production-manager selection, real-installer mid-move rollback, and peer-byte retention in both directions); **CS-09/10 pushed manager-contract candidate, independently reviewed with conditions and CI green** (last active scheme falls back to Luna; retained peer is reselected and receives a successful controlled deployment request without reinstall); **CS09-10-01 local candidate awaiting independent review** (real installer inventory and retained-peer byte proof in both directions). Existing P4 production transaction is reused; this slice adds dual-scheme evidence. Isolation checkout: `/private/tmp/uk-scheme-delivery-fix` on `codex/scheme-delivery-fix`. PR #100 remains **draft**. Wanxiang P4 / ADR 0034 Proposed / TestFlight 仍开放 |
| Material non-claims | Push of the reviewed candidate was Human-authorized; no TestFlight/App Release; no full Product Gate Passed; no Device-attested upgrade; no ADR 0034 acceptance; no undraft/merge of PR #100; no Wanxiang P4 closure; no peer-prefer fallback B; no Recovery persistence |
| Next handoff / decision | `CSF-PAIR-01/02` is pushed, CI green, and independently reviewed. `CS09-10-01` local candidate awaits independent review; device/runtime evidence (`CS09-10-02`), PR #100 push/undraft/merge, TestFlight, App Release and ADR Accept remain separate human decisions. |
| Residuals | CS09-10-01 的本地固定解压树 inventory 证明不重新验证 archive provenance，且不覆盖真实 RIME 输入、App Group 或 device transaction；`CS09-10-02` 未做；CSF pair 的真实 RIME runtime input、App Group/device transaction 未证明；真机失败回滚未测（Limited Gate 历史）；无 Device-attested；Wanxiang P4 未全闭合；Ice Lua `dofile` 动态引用未闭合；backup/staging cleanup 仍 best-effort |
