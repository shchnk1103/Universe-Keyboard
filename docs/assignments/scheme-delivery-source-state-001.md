...
| Next handoff / decision | After hosted CI green on the docs-authorization tip: undraft+merge PR #100 (Human authorized). Then open separate Assignments for `RTRD-01` diagnostics UI and/or `RTRD-02` elapsed measurement as needed; Product Gate / TestFlight / ADR Accept remain separate Human decisions. Do **not** continue diagnostics UI or elapsed work inside this PR without a new Assignment. |
