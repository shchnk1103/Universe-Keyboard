...
**2026-09-08 current increment:** Cross-scheme **CS-03–CS-08 plus CS-F1/CS-F3** is pushed to `codex/scheme-delivery-fix` at `f8e297e` (PR #100 remains draft): identical-receipt idempotent no-op, inactive-uninstall peer-retain, active-uninstall Luna-only peer coverage, and named failure injections in `SchemaManagerTests` and `SchemeResourcePreparationCoexistenceTests`. CS-F1/CS-F3 received an [independent Pass-with-conditions review](../reviews/scheme-delivery-cross-scheme-csf1-csf3-2026-09-08.md). **CS-09/10** is a local manager-contract candidate with an [independent Pass-with-conditions review](../reviews/scheme-delivery-cross-scheme-cs09-cs10-2026-09-08.md). Base tip before the matrix work: `944477a`. CS-01/02 already on branch (`2813428`). Human superseded peer-prefer B with Luna-only fallback.
...
|---|---|
| Lifecycle | Active |
| Current Phase | Cross-scheme matrix: **CS-01/02 done** (committed `2813428`); **CS-03/04 done locally** (identical no-op + peer retain / Wanxiang identity-change upgrade path; eng `cb06a08`); **CS-05/06 done locally** (inactive uninstall retains active peer and does not force Luna); **CS-07/08 done locally** (active uninstall always deploys Luna before target removal); **CS-F1/CS-F3 local candidate, independently reviewed with conditions** (named dual-scheme deployment/staging failures cover peer and receipt boundaries; CS-F3 production selection plus real-installer failure remains a residual); **CS-09/10 local manager-contract candidate, independently reviewed with conditions** (last active scheme falls back to Luna; retained peer is reselected and receives a successful controlled deployment request without reinstall). Existing P4 production transaction is reused; this slice adds dual-scheme evidence. Isolation checkout: `/private/tmp/uk-scheme-delivery-fix` on `codex/scheme-delivery-fix`. PR #100 remains **draft**. Wanxiang P4 / ADR 0034 Proposed / TestFlight 仍开放 |
...
| Next handoff / decision | CS-09/10 independent review recorded as **Pass with conditions**. Pause for separately authorized `CS09-10-01`, `CSF-PAIR-01/02` residual work, device evidence (`CS09-10-02`), PR #100 undraft/merge, TestFlight, App Release or ADR Accept. |
| Residuals | CS-09/10 的 production installer 文件 inventory 与真实 RIME 输入未在本切片重测（`CS09-10-01/02`）；CS-F1/CS-F3 have Ice-target coverage only (`CSF-PAIR-01`); CS-F3 尚未把生产 `SchemaManager` 的 selection 保持断言与真实 installer 的 mid-move failure 合并证明（`CSF-PAIR-02`）；真机失败回滚未测（Limited Gate 历史）；无 Device-attested；Wanxiang P4 未全闭合；Ice Lua `dofile` 动态引用未闭合；backup/staging cleanup 仍 best-effort |
