...
**2026-09-08 current increment:** Cross-scheme **CS-03–CS-08 plus CS-F1/CS-F3** is pushed to `codex/scheme-delivery-fix` at `f8e297e` (PR #100 remains draft): identical-receipt idempotent no-op, inactive-uninstall peer-retain, active-uninstall Luna-only peer coverage, and named failure injections in `SchemaManagerTests` and `SchemeResourcePreparationCoexistenceTests`. CS-F1/CS-F3 received an [independent Pass-with-conditions review](../reviews/scheme-delivery-cross-scheme-csf1-csf3-2026-09-08.md). **CS-09/10** is a local test-only candidate awaiting independent review. Base tip before the matrix work: `944477a`. CS-01/02 already on branch (`2813428`). Human superseded peer-prefer B with Luna-only fallback.
...
| Next handoff / decision | CS-09/10 independent review pending. Then pause for separately authorized CSF-PAIR-01/02 residual work, device evidence, PR #100 undraft/merge, TestFlight, App Release or ADR Accept. |
| Residuals | CS-09/10 的 production installer 文件 inventory 与真实 RIME 输入未在本切片重测；CS-F1/CS-F3 have Ice-target coverage only (`CSF-PAIR-01`); CS-F3 尚未把生产 `SchemaManager` 的 selection 保持断言与真实 installer 的 mid-move failure 合并证明（`CSF-PAIR-02`）；真机失败回滚未测（Limited Gate 历史）；无 Device-attested；Wanxiang P4 未全闭合；Ice Lua `dofile` 动态引用未闭合；backup/staging cleanup 仍 best-effort |
