...
**2026-09-08 current increment:** Cross-scheme **CS-03–CS-10 plus CS-F1/CS-F3** is pushed to `codex/scheme-delivery-fix` at `fd8ba55` and GitHub CI is green (PR #100 remains draft): identical-receipt idempotent no-op, inactive-uninstall peer-retain, active-uninstall Luna-only peer coverage, final-scheme Luna fallback, retained-peer redeploy coverage, and named failure injections in `SchemaManagerTests` and `SchemeResourcePreparationCoexistenceTests`. The separately authorized `CSF-PAIR-01/02` addition is pushed at `a6771ed` with GitHub CI green: symmetric Wanxiang F1 and combined manager/real-installer F3 proof in both directions, focused and full App + Keyboard tests, and an independent Pass-with-conditions review. Separately authorized `CS09-10-01` is pushed at `6da6267`, independently reviewed Pass-with-conditions, and GitHub CI green as Human-reported: real installer inventory and retained-peer byte proof in both directions passed focused and full App + Keyboard tests. CS-F1/CS-F3 and CS-09/10 each received an independent Pass-with-conditions review. Base tip before the matrix work: `944477a`. CS-01/02 already on branch (`2813428`). Human superseded peer-prefer B with Luna-only fallback.
...
| Next handoff / decision | `CSF-PAIR-01/02` and `CS09-10-01` are pushed, CI green, and independently reviewed. `CS09-10-02` recorded a human-attested failure in both directions; a layout-independent [runtime-route reconciliation proposal](../plans/scheme-delivery-active-uninstall-runtime-route-reconciliation-2026-09-08.md) awaits separate implementation authorization. PR #100 undraft/merge, TestFlight, App Release and ADR Accept remain separate human decisions. |
| Residuals | [CS09-10-02](../evidence/scheme-delivery-cross-scheme-cs09-cs10-device-2026-09-08.md) failed: Luna has no candidate input after active Ice or Wanxiang uninstall. The Proposed route reconciler requires Architecture/Quality review and a fresh device matrix; CS09-10-01 still does not reverify archive provenance and retains its P2/P3 limits; CSF pair 的真实 RIME runtime input、App Group/device transaction 未证明；真机失败回滚未测（Limited Gate 历史）；无 Device-attested；Wanxiang P4 未全闭合；Ice Lua `dofile` 动态引用未闭合；backup/staging cleanup 仍 best-effort |
