...
**2026-09-08 current increment:** Cross-scheme **CS-03–CS-10 plus CS-F1/CS-F3** is pushed to `codex/scheme-delivery-fix` at `fd8ba55` and GitHub CI is green (PR #100 remains draft): identical-receipt idempotent no-op, inactive-uninstall peer-retain, active-uninstall Luna-only peer coverage, final-scheme Luna fallback, retained-peer redeploy coverage, and named failure injections in `SchemaManagerTests` and `SchemeResourcePreparationCoexistenceTests`. The separately authorized `CSF-PAIR-01/02` local candidate adds symmetric Wanxiang F1 and combined manager/real-installer F3 proof in both directions; focused and full App + Keyboard tests pass, with an independent Pass-with-conditions review. CS-F1/CS-F3 and CS-09/10 each received an independent Pass-with-conditions review. Base tip before the matrix work: `944477a`. CS-01/02 already on branch (`2813428`). Human superseded peer-prefer B with Luna-only fallback.
...
| Next handoff / decision | `CSF-PAIR-01/02` local candidate is independently reviewed. Pause for separately authorized `CS09-10-01`, device evidence (`CS09-10-02`), PR #100 push/undraft/merge, TestFlight, App Release or ADR Accept. |
| Residuals | CS-09/10 的 production installer 文件 inventory 与真实 RIME 输入未在本切片重测（`CS09-10-01/02`）；CSF pair 的 complete pinned-resource inventory、真实 RIME runtime input、App Group/device transaction 未证明；真机失败回滚未测（Limited Gate 历史）；无 Device-attested；Wanxiang P4 未全闭合；Ice Lua `dofile` 动态引用未闭合；backup/staging cleanup 仍 best-effort |
