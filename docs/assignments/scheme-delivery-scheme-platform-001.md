...
|---|---|
| Lifecycle | **Active** |
| Current Phase | **P1-0 complete**（`2026-09-10 Asia/Shanghai`）：Human-approved P1 extract order **P1-0→P1-6** landed — [`scheme-platform-p1-extract-plan-2026-09-10.md`](../plans/scheme-platform-p1-extract-plan-2026-09-10.md)（含 Ice regression checklist）。P0 仍有效：[`scheme-platform-p0-interfaces-2026-09-09.md`](../plans/scheme-platform-p0-interfaces-2026-09-09.md) + [`scheme-platform-p0-ice-wanxiang-matrix-2026-09-09.md`](../evidence/scheme-platform-p0-ice-wanxiang-matrix-2026-09-09.md)。冻结 tip base：`origin/main` @ `814abfd7c03002256978d7658c176b80002d2539`；工作分支 `codex/scheme-platform-001`（draft PR #102 — **本片仅本地 commit，不 push**）。Wanxiang P4 **Paused**（A34-R1 open — NOT Closed/Done）。**无** ADR Accept；**无** Product Gate / TF；**无** Swift in P1-0。下一动作：**ask Human before P1-1 Swift**；push/PR 仍须先问 Human。KOS：local commit per slice；ask before push；no ADR Accept。 |
| Material non-claims | **不是** ADR 0034 Accept；**不是** Product Gate / TestFlight / Release；**不是** P1-1+ Swift 授权（P1-0 = docs plan only；P1-1 Swift 须另问 Human）；不把 Wanxiang 内容改写成 Ice；**不** enable Wanxiang nine-key；**不** Discovery UI；**不**自动 Closed Wanxiang P4 / A34-R1（Pause ≠ Done）；本片 **不 push** |
| Next handoff / decision | P1-0 plan + Ice regression checklist 已本地落盘。**ask before P1-1 Swift**；**ask before push/PR**（draft #102 不自动更新）。Human ADR/#101：leave draft #101；keep #101≠#102；**no Accept ADR 0034 now**。ADR Accept / Product Gate / TF / A34-R1 writeback 均须另授权。 |
