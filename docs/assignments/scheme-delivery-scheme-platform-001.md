...
|---|---|
| Lifecycle | **Active** |
| Current Phase | **P1 Done + IQ recorded**（`2026-09-12 Asia/Shanghai`）：P1-0…P1-5 extract complete；P1-6 summary [`scheme-platform-p1-summary-2026-09-12.md`](../plans/scheme-platform-p1-summary-2026-09-12.md) + Independent Quality [`scheme-delivery-scheme-platform-001-p1-quality-review.md`](../reviews/scheme-delivery-scheme-platform-001-p1-quality-review.md) — **Pass with conditions** on freeze tip `3cfa355`。**Keep Platform Active**（Exit Criteria 未满 — P2/P3 pending）；**无** Assignment Close；**无** ADR Accept；**无** Product Gate/TF。分支 `codex/scheme-platform-001`（draft #102 — **P1-6 docs local commit only；ask before push**）。下一：**P2 awaiting Human auth**（Wanxiang → `privatePreset`；ownership stays dual）；**ask before push**。 |
| Material non-claims | **不是** ADR 0034 Accept；**不是** Product Gate / TestFlight / Release；**不是** Assignment Close / Exit 已满足；**不** enable Wanxiang nine-key；**不** Discovery UI productization；**不**自动 Closed Wanxiang P4 / A34-R1（仍 `fix`；Pause ≠ Done）；P2 Wanxiang `privatePreset` migrate **另授权**；**不** silent-close IQ residuals（SP-P1-IQ-01…）；**不** push P1-6 tip without Human ask；leave draft #101；keep #101≠#102 |
| Next handoff / decision | P1-6 summary + IQ landed locally（freeze `3cfa355`；Verdict **Pass with conditions**）。**ask before push** of P1-6 docs tip to draft #102。下一 = **P2** Wanxiang platform migrate（SharedDefault → `privatePreset`；ownership stays `exactHash`）— **awaiting Human auth**。Human ADR/#101：leave draft #101；keep #101≠#102；**no Accept ADR 0034 now**；**no** Product Gate/TF；**no** Assignment Close。Watch SP-P1-IQ-01（`3cfa355` hosted `build-and-test`）。 |
| Residuals | IQ：SP-P1-IQ-01（`3cfa355` hosted `build-and-test` pending at review）`open`；SP-P1-IQ-02 malloc flake `accept`；fixture skips / Discovery deferred / P2 migrate open — see IQ residuals table。A34-R1 仍 open（`fix`）于 Paused Wanxiang P4；A34-R2 / TD-011 / RTRD-* 并行债 |
