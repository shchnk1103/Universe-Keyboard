...
|---|---|
| Lifecycle | **Active** |
| Current Phase | **P1-1 landed locally**（`2026-09-10 Asia/Shanghai`）：`SchemeAdapter` registry + Ice/Wanxiang/Luna adapters mirroring today’s hardcodes；KeyboardCore equality tests；optional thin `postProcessingRevision` → registry。**未**替换 `isNineKeyCapable` 体（P1-2）；**未** SharedDefault post-process（P1-3）；**未** exactHash installer（P1-4）。P1-0 plan 仍有效：[`scheme-platform-p1-extract-plan-2026-09-10.md`](../plans/scheme-platform-p1-extract-plan-2026-09-10.md)。分支 `codex/scheme-platform-001`（draft #102 — **local only；ask before push**）。**无** ADR Accept；**无** Wanxiang nine-key enable；**无** Discovery UI。下一：P1-2 LayoutCapability（另授权 / 按序）；**ask before push**。 |
| Material non-claims | **不是** ADR 0034 Accept；**不是** Product Gate / TestFlight / Release；**不** enable Wanxiang nine-key；**不** Discovery UI；**不**自动 Closed Wanxiang P4 / A34-R1（Pause ≠ Done）；P1-1 **不**改 product behavior / **不** push；P1-2+ 仍按序 |
| Next handoff / decision | P1-1 registry 已本地 commit。**ask before push/PR**（draft #102）；下一 Swift = **P1-2** LayoutCapability（Ice T9 unchanged；Wanxiang nine=false）。Human ADR/#101：leave draft #101；keep #101≠#102；**no Accept ADR 0034 now**。 |
