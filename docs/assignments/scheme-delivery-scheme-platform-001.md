...
|---|---|
| Lifecycle | **Active** |
| Current Phase | **P1-2 done locally this tip**（`2026-09-11 Asia/Shanghai`）：`LayoutCapability`（`nineKeySchemaIDs`）+ `RimeRuntimeSelection.isNineKeyCapable` / `isTwentySixKeyCapable`（及 binding9 / ADR 0018 family migration）经 `SchemeAdapterRegistry`；Ice T9 unchanged；Wanxiang `supportsNineKey=false`。**P1-1 pushed on #102**（CI green at `a092de8`）。**未** SharedDefault post-process（P1-3）；**未** exactHash installer（P1-4）。P1-0 plan 仍有效：[`scheme-platform-p1-extract-plan-2026-09-10.md`](../plans/scheme-platform-p1-extract-plan-2026-09-10.md)。分支 `codex/scheme-platform-001`（draft #102 — **local only this tip；ask before push**）。**无** ADR Accept；**无** Wanxiang nine-key enable；**无** Discovery UI。下一：P1-3 SharedDefault（另授权 / 按序）；**ask before push**。 |
| Material non-claims | **不是** ADR 0034 Accept；**不是** Product Gate / TestFlight / Release；**不** enable Wanxiang nine-key；**不** Discovery UI；**不**自动 Closed Wanxiang P4 / A34-R1（Pause ≠ Done）；P1-2 **不**改 product behavior / **不** push；P1-3+ 仍按序 |
| Next handoff / decision | P1-2 LayoutCapability 已本地 commit（base tip `a092de8` + this tip）。**ask before push/PR**（draft #102）；下一 Swift = **P1-3** SharedDefault Ice `privatePreset` seam。Human ADR/#101：leave draft #101；keep #101≠#102；**no Accept ADR 0034 now**。 |
...
- `2026-09-11 Asia/Shanghai`（P1-2）：LayoutCapability + route `isNineKeyCapable` / `isTwentySixKeyCapable` via SchemeAdapter registry；Ice T9 unchanged；Wanxiang nine=false；local commit only；ask before push；next P1-3 awaiting auth。
...
| `2026-09-11 Asia/Shanghai` | **P1-2** | LayoutCapability（`nineKeySchemaIDs`）+ route `RimeRuntimeSelection`/`SchemaManager` guards via SchemeAdapter registry；Ice T9 unchanged；Wanxiang `supportsNineKey=false`；registry + binding9 tests；**local commit only**；**ask before push**；next P1-3 awaiting auth；**no** ADR Accept；**no** Wanxiang nine-key；**no** Discovery UI |
...
- `2026-09-11 Asia/Shanghai`（**P1-2**）：LayoutCapability live authority — `nineKeySchemaIDs` on adapters；`RimeRuntimeSelection.isNineKeyCapable` / `isTwentySixKeyCapable` (+ binding9 guard / ADR 0018 `familySupportsNineKey`) route through `SchemeAdapterRegistry`. Ice T9 unchanged；Wanxiang nine=false. P1-1 already on draft #102 tip `a092de8`（CI green）. **Local commit only this tip**；**ask before push**；next P1-3 awaiting auth；**no** ADR Accept；**no** Wanxiang nine-key；**no** Discovery UI.
