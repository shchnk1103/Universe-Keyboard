...
|---|---|
| Lifecycle | **Active** |
| Current Phase | **P1-3 done locally this tip**（`2026-09-11 Asia/Shanghai`）：SharedDefault seam — `SchemeSharedDefaultApplying` + registry `sharedDefaultApplicator` / `applySharedDefaultPostExtract`；Ice `privatePreset` → `RimeIceSharedDefaultAdapter`；Download post-process 经 registry lookup（不再 `schemaID == "rime_ice"` 调 SharedDefault）；Wanxiang 仍 `consumePrelude` no-op（**未** migrate privatePreset）。**P1-2 pushed on #102** tip `5bba4d2`（CI green）。**未** ResourceOwnership installer（P1-4）。P1-0 plan 仍有效：[`scheme-platform-p1-extract-plan-2026-09-10.md`](../plans/scheme-platform-p1-extract-plan-2026-09-10.md)。分支 `codex/scheme-platform-001`（draft #102 — **local only this tip；ask before push**）。**无** ADR Accept；**无** Wanxiang nine-key enable；**无** Discovery UI。下一：P1-4 ResourceOwnership（另授权 / 按序）；**ask before push**。 |
| Material non-claims | **不是** ADR 0034 Accept；**不是** Product Gate / TestFlight / Release；**不** enable Wanxiang nine-key；**不** Discovery UI；**不**自动 Closed Wanxiang P4 / A34-R1（Pause ≠ Done）；P1-3 **不** migrate Wanxiang preset（P2）；**不** push this tip；P1-4+ 仍按序 |
| Next handoff / decision | P1-3 SharedDefault Ice `privatePreset` seam 已本地 commit（P1-2 tip `5bba4d2` CI green on #102）。**ask before push/PR**（draft #102）；下一 Swift = **P1-4** ResourceOwnership `namedList`+`exactHash`。Human ADR/#101：leave draft #101；keep #101≠#102；**no Accept ADR 0034 now**。 |
...
- `2026-09-11 Asia/Shanghai`（P1-3）：SharedDefault Ice `privatePreset` seam via SchemeAdapter registry；Wanxiang consumePrelude unchanged；local commit only；ask before push；next P1-4 awaiting auth。
...
| `2026-09-11 Asia/Shanghai` | **P1-3** | SharedDefault seam（`SchemeSharedDefaultApplying` + registry applicator）；Ice `privatePreset` → `RimeIceSharedDefaultAdapter`；Download post-process via registry；Wanxiang `consumePrelude` no-op；Ice preset coexistence tests；**local commit only**；**ask before push**；next P1-4 awaiting auth；**no** ADR Accept；**no** Wanxiang preset migrate；**no** Discovery UI |
