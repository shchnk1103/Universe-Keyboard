...
|---|---|
| Lifecycle | **Active** |
| Current Phase | **P1-5 done locally this tip**（`2026-09-12 Asia/Shanghai`）：UninstallHooks / `onUninstallPrepare` — Ice 仍走今日 `prepareRimeIceUninstallWithLayoutFallback` 语义（26-key + invalidate T9 readiness）via SchemeAdapter registry；**无** Discovery Section A-only picker / full A/B fallback productization。**P1-4 on #102** tip `5c42546`/`82bdd60`（merge main；hosted CI fully green）。P1-0 plan 仍有效：[`scheme-platform-p1-extract-plan-2026-09-10.md`](../plans/scheme-platform-p1-extract-plan-2026-09-10.md)。分支 `codex/scheme-platform-001`（draft #102 — **local only this tip；ask before push**）。**无** ADR Accept；**无** Wanxiang nine-key；**无** Discovery UI；**未** start P1-6 IQ（docs pointer only）。下一：P1-6 summary+IQ（另授权）；**ask before push**。 |
| Material non-claims | **不是** ADR 0034 Accept；**不是** Product Gate / TestFlight / Release；**不** enable Wanxiang nine-key；**不** Discovery UI / Section A-only picker / full A/B fallback productization；**不**自动 Closed Wanxiang P4 / A34-R1（Pause ≠ Done）；P1-3 **不** migrate Wanxiang preset（P2）；P1-4 **不** whole-dir wipe / filename-heuristic auto-remove；P1-5 **不** Discovery UI；**不** start P1-6 IQ（除 docs pointer）；**不** push this tip；P1-6 仍按序另授权 |
| Next handoff / decision | P1-5 UninstallHooks Ice layout fallback 已本地 commit（P1-4 tip `5c42546`/`82bdd60` CI green on #102）。**ask before push/PR**（draft #102）；下一 = **P1-6** summary + Independent Quality（另授权；no silent conditional close）。Human ADR/#101：leave draft #101；keep #101≠#102；**no Accept ADR 0034 now**。 |
