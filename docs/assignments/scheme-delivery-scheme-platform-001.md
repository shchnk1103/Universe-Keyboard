...
|---|---|
| Lifecycle | **Active** |
| Current Phase | **P1-4 done locally this tip**（`2026-09-11 Asia/Shanghai`）：ResourceOwnership / ResourceCapability — Ice `namedList` + Wanxiang `exactHash` strategies via SchemeAdapter registry；uninstall / upgrade-checkpoint staging 经 strategies（行为保持）；Wanxiang OpenCC `admitted=false`；**无** whole-dir wipe / filename-heuristic auto-remove。**P1-3 on #102** tip `22c6b79`/`d8e8299`（merge main；hosted CI green）。P1-0 plan 仍有效：[`scheme-platform-p1-extract-plan-2026-09-10.md`](../plans/scheme-platform-p1-extract-plan-2026-09-10.md)。分支 `codex/scheme-platform-001`（draft #102 — **local only this tip；ask before push**）。**无** ADR Accept；**无** Wanxiang nine-key；**无** Discovery UI；**未** start P1-5。下一：P1-5 Uninstall hook shape（另授权）；**ask before push**。 |
| Material non-claims | **不是** ADR 0034 Accept；**不是** Product Gate / TestFlight / Release；**不** enable Wanxiang nine-key；**不** Discovery UI；**不**自动 Closed Wanxiang P4 / A34-R1（Pause ≠ Done）；P1-3 **不** migrate Wanxiang preset（P2）；P1-4 **不** whole-dir wipe / filename-heuristic auto-remove；**不** start P1-5；**不** push this tip；P1-5+ 仍按序 |
| Next handoff / decision | P1-4 ResourceOwnership dual strategies 已本地 commit（P1-3 tip `22c6b79`/`d8e8299` CI green on #102）。**ask before push/PR**（draft #102）；下一 Swift = **P1-5** Uninstall hook shape（Ice layout fallback only；另授权）。Human ADR/#101：leave draft #101；keep #101≠#102；**no Accept ADR 0034 now**。 |
