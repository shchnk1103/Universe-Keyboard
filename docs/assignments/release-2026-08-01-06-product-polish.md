...
- **Release feature gate:** Contextual typo correction's delayed candidate refresh is compiled only in `DEBUG`. Release builds retain normal RIME candidates and the included post-commit continuation, but do not inject contextual-correction candidates after the user pauses typing.
- **Verification:** `bash scripts/ensure_rime_vendor.sh verify` passed after restoring the already-pinned local Vendor artifacts to this isolated worktree. Debug and Release builds of `Universe Keyboard` for the iPad Pro 11-inch (M5) simulator succeeded. The launched Debug Home screenshot retained the card without a visible navigation affordance. The iPad simulator test run was repeated after the final feature-gate change and completed with `121` passed, `0` failed and `0` skipped.
