...
14. For notifications, verify `rimeSync notification authorization completed` and `rimeSync notification scheduled`; `notification skipped authorizationStatus=` means system permission is denied or has not been requested. Confirm the notification subject matches the path that actually ran: RIME standard data, Universe settings or both. Payloads must never include paths, dictionary entries, recovery codes or input content.
