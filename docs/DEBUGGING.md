...
4. Verify the final commit produced `.continuationChanged` and candidate presentation reset its snapshot.
5. Confirm no intervening newline, host deletion, English-mode switch, visibility change or setting disable cleared the state.
6. Do not log the retained context or candidate text. Inspect only eligibility flags, counts and state transitions.
