...
Last synced: `2026-08-29 Asia/Shanghai` — RIME 后台同步崩溃修复 PR #91 保持 Draft；hosted CI 与强制真机启动通过，独立双审为 `Pass with conditions`，自然调度与生命周期 residual 仍开放。核心 Active Work 为 `4/10`。
...
| 4 | RIME-SYNC-001 | Active | PR #91 `e3e5d77` hosted CI 与强制真机后台启动通过；Architecture/Quality 均为 `Pass with conditions`；自然调度、手机通知呈现、生命周期 residual 与 Product Gate pending | [`assignment`](assignments/rime-sync-001.md) · [`evidence`](evidence/rime-background-sync-crash-fix-2026-08-29.md) · [`Architecture`](assignments/rime-sync-001-background-sync-crash-architecture-review.md) · [`Quality`](assignments/rime-sync-001-background-sync-crash-quality-review.md) |
