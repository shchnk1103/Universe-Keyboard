...
Last synced: `2026-08-28 Asia/Shanghai` — TD-016-CI-TIERING-001 已获 Product 授权并进入 Active；核心 Active Work 为 `4/10`。万象 v1 打点继续由 TD-015 追踪，不占 Active。
...
| 4 | TD-016-CI-TIERING-001 | Active | 独立 Architecture/Quality review 为 `Pass with conditions`；修复后复核并补 docs-only hosted fixture | [`assignment`](assignments/td-016-ci-tiering-001.md) · [`Architecture`](reviews/TD-016-CI-TIERING-001-architecture-review.md) · [`Quality`](reviews/TD-016-CI-TIERING-001-quality-review.md) |
...
| P2 | **[TD-016](TECH_DEBT.md#td-016-ci-变更分级与文档提交快速门禁) CI 变更分级** | 两次 full-path 已绿；独立 review 条件修复中，之后复核并补 docs-only hosted fixture；未改 required checks |
