...
| Next review | `v0.6.0` re-review after the RIME background-sync thread freezes a checkpoint / merges and when a workflow will use multi-agent or multi-provider orchestration; or when enabling `required` / fixing [`TD-014`](../TECH_DEBT.md#td-014-kos-22-auth-consumption_state-卫生) |
...
- `v0.6.0`（可选 AI 编排合同）：`Deferred`，见 [`KOS-UPGRADE-UK-002-v0.6.0`](upgrade-records/KOS-UPGRADE-UK-002-v0.6.0.md)。理由：当前无 workflow 需要多 agent / 多 provider 编排；且 RIME 后台同步线程在本仓库共享治理文件上 Active、临近 merge，不应在重叠 writer 运行时改共享采用文件。待其冻结 checkpoint / merge、且确有编排需求时再复核；既有 Active Assignment 保持 pinned、不迁移。`v0.6.0` 不改冻结 `core/`、不加 schema 强制，不改变本项目的 advisory 义务。
