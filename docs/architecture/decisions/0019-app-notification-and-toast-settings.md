...
- 系统通知采用“总开关 + 类别开关 + 类别内通知子项”。常规类别为 RIME 云同步，包含“RIME 标准同步”和“Universe 设置同步”两个通知子项；RIME 页面和全局页面复用同一状态与控件。Debug-only 的“首屏高保真诊断结束”是独立类别，默认关闭，并在诊断页与全局通知页复用同一选择。
...
- 主 App 在开启首屏高保真窗口时，以固定标识安排一条基于绝对过期时间的本地通知；窗口取消、类别关闭或总开关关闭时移除同一待发送请求。它不包含诊断事件、日志字段或任何用户输入内容。
