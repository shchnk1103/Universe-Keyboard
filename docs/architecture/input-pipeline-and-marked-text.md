...
- **T9 Path residual-B cursor (004 Gate 5):** after the user Path-selects a stack and partials a candidate of `K` CJK characters, Core peels `K` user Path syllables (digit slots follow those syllables), focuses the next user-stack syllable with soft-select only if the user had selected it, and leaves unselected tails (`wo…`) without forged selection. SoT: [`PD-…-GATE5-RESIDUAL-B-PATH-LEDGER-PEEL`](../product-decisions/KEYBOARD-LAYOUT-9KEY-PINYIN-004-gate5-residual-b-path-ledger-peel.md); architecture note in [`partial-commit.md`](partial-commit.md).
