...
After a successful initial sync, automatic standard sync is enabled by default. The user can turn it off or choose a daily or seven-day cooldown. The containing App only asks iOS for a background opportunity; iOS may delay or skip it. Before an automatic run, the App checks that the selected folder is still authorized and that the keyboard is not active. If the user separately permits local notifications, the App may show “开始自动同步” and “自动同步完成”; those notifications contain no directory, dictionary, recovery code or typed content.
