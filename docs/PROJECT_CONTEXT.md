...
- **`BundledContinuationSuggestionProvider`** — validates and decodes a small versioned bundled resource once, then performs bounded longest-suffix lookup in memory. V1.2 retains fail-closed size/structure validation and protects 60 reviewed synthetic Top-3 cases across 15 categories through [`POST_COMMIT_CONTINUATION_QUALITY.md`](POST_COMMIT_CONTINUATION_QUALITY.md). It never reads host context or persists committed text.
