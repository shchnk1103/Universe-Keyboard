...
- **Typo correction learning is not RIME learning**: V0.8b may learn only from explicit selection of an already validated insertion correction candidate. Store bounded correction-pair metadata locally in App Group preferences; never store surrounding text, upload telemetry, write RIME user dictionaries, or let learned history bypass correction assessment. Release builds keep insertion disabled by default.
