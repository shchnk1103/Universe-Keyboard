...
- **`BundledContinuationSuggestionProvider`** — validates and decodes a small versioned bundled resource once, then performs bounded longest-suffix lookup in memory. V1.3 retains fail-closed size/structure validation and protects reviewed Top-3, exact Top-1 naturalness and ambiguous-suffix suppression cases through [`POST_COMMIT_CONTINUATION_QUALITY.md`](POST_COMMIT_CONTINUATION_QUALITY.md). It never reads host context or persists committed text.
...
- **Activation / Full Access onboarding**: main-App Settings toolbar **？** opens one activation sheet (J0–J5). No Help tab. Incomplete/recovery marks **？** and auto-presents on next process launch; 「稍后再说」 only ends the current process sheet. J4 trial field is in-sheet. Product source: [`ONBOARDING_ACTIVATION.md`](ONBOARDING_ACTIVATION.md); semantics: [`PD-RELEASE-2026-0801-03`](product-decisions/RELEASE-2026-0801-03-activation-authorization.md); presentation: [`PD-HELP-TIPKIT-001`](product-decisions/HELP-TIPKIT-001-authorization.md) / [`PD-HELP-GUIDE-SHEET-001`](product-decisions/HELP-GUIDE-SHEET-001-authorization.md). J3 slim embed: [`PD-HELP-J3-RESOURCES-001`](product-decisions/HELP-J3-RESOURCES-001-authorization.md). Always-on **搜索** tab (far right) for settings search: [`PD-APP-SEARCH-001`](product-decisions/APP-SEARCH-001-authorization.md). The main App must not invent a live Extension Full Access flag; TipKit is optional packaging; no Extension tips. Implementation Assignment [`HELP-GUIDE-SHEET-001`](assignments/help-guide-sheet-001.md) is Closed.
...
- **Partial Commit**: selecting a shorter RIME candidate inside active composition may keep remaining composition active, with reversible Delete-restore semantics. Feature matrix, eligibility gates, typo-correction interaction, checkpoint contract, and T9 Path residual-B cursor (user Path stack + soft-select after multi/single CJK partial): [`partial-commit.md`](architecture/partial-commit.md) and [`input-pipeline-and-marked-text.md`](architecture/input-pipeline-and-marked-text.md) §Partial Commit Invariants. Residual-B product SoT: [`PD-…-GATE5-RESIDUAL-B-PATH-LEDGER-PEEL`](product-decisions/KEYBOARD-LAYOUT-9KEY-PINYIN-004-gate5-residual-b-path-ledger-peel.md) (Human Pass; PR #28).
...
- **Diagnostics**: `Logger` (KeyboardCore, `Sendable`, FIFO serial writer, 500-entry ring buffer) writes to App Group `UserDefaults`. Main App **设置 → 诊断**: status + master switch + always-mounted categories (disabled when off) + log viewer; nine-key force_gc tools under **高级**. Viewer: `DiagnosticsView` (filter/copy/clear). Procedures: [`DEBUGGING.md`](DEBUGGING.md).
