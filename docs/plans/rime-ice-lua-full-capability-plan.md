...
- 用户能在主 App 中理解「高级输入功能」是什么。
- 用户能看到当前设备是否支持这些增强功能。
- 用户能按功能开启或关闭增强功能，并知道修改后需要重新部署 RIME。
- 当当前方案不支持高级输入功能时，开关不可调整，但用户的选择会保留到支持的方案中生效。
...
- 已将 Lua 文件完整性诊断扩展为从 `rime_ice.schema.yaml` 的 `lua_processor` / `lua_segmentor` / `lua_translator` / `lua_filter` 引用反推 `lua/*.lua` 文件，不只检查单个日期脚本。
...
- 已通过真机主 App 部署 smoke 和手动输入验证 `rq` / `sj` / `xq` / `dt` 的动态候选路径。
- 尚未完成完整自动化 `rime_ice` Lua smoke test runtime fixture；`ts`、`R123`、`cC1+1`、Unicode、UUID、农历等能力仍需补充验收记录。

2026-06-19：

- 已增加全局 `高级输入功能` 设置页，入口位于设置页；RIME 方案设置页保留方案管理和部署状态。
- 已按普通用户能理解的功能分组展示开关，并隐藏 `Lua`、`translator`、`filter` 等内部实现词。
- 已在当前方案不支持高级输入时禁用开关，并说明用户选择会保留到支持的方案中使用。
- 已增加可逆的高级输入后处理：关闭功能时部署阶段禁用对应动态候选组件，重新开启时从保留的原始方案配置恢复。
- 已在高级输入功能页增加 `rq` / `sj` / `xq` / `dt`、计算、数字格式和特殊内容的使用说明。
- 已修复主 App 打开时读取旧部署状态误弹「部署成功」toast 的问题；现在只有本次会话真实触发部署后才显示部署结果。
- 已补充本地自动化覆盖：单项开关的组件映射、总开关禁用、可逆恢复、非支持方案状态说明、部署签名和完整动态组件解析。
- 已通过用户真机回归确认普通拼音数字组合、删除顺序和候选栏清理行为正常。
- 未纳入本次实现阻塞的发布前补证据：完整自动化 `rime_ice` runtime fixture、VoiceOver、Dynamic Type、浅色/深色截图，以及更多代表输入的手动记录。
...
方案详情页只展示摘要和恢复入口，不重复完整开关列表。

原因：

- 用户选择的是「我是否想要日期、计算器、数字转换等高级功能」，不是某个方案详情页里的私有状态。
- 未来多个方案支持部分高级功能时，统一入口能避免每个方案详情页重复一套开关。
- 实际生效能力由「用户偏好 ∩ 当前方案支持能力 ∩ 已部署状态」决定。
...
- `日期与时间`：日期、时间、星期、农历。
- `计算与数字`：计算器、数字大写、金额格式。
- `编码与工具`：特殊字符编码、随机编号、V 模式辅助、辅助查询。
- `候选优化`：输入提示、长词候选优化、拼音候选优化、英文候选降噪。
- `输入辅助`：选字辅助、英文大小写辅助。
...
- `当前方案暂不支持`：当前 active schema 没有声明高级输入能力，开关保留但不可调整。
...
- 在设置顶层/RIME 设置页增加 `高级输入功能` 入口。
- 开关按用户能理解的功能分组，而不是暴露内部组件名。
- 当前方案不支持时，开关禁用并显示普通用户能理解的说明。
- 开关关闭时，通过可逆的受控补丁禁用对应动态候选组件。
- 开关开启时，从保留的原始 scheme 配置恢复完整能力。
- 切换后标记 `rime_needs_deploy = true`，并引导用户重新部署，或沿用现有自动部署模式。

完成标准：

- 用户可以在支持的方案下开启/关闭每一类增强功能。
- 不支持的方案下设置不可调整，且有清晰说明。
- 开关文案普通用户可理解。
- 设置变化不会在 Keyboard Extension 输入热路径中写文件。

### 阶段 5：真实 Lua 烟测（实现已完成，发布前可继续补证据）
...
- smoke test 骨架能在本地 simulator 跳过或使用真实 runtime fixture 运行。
- 失败时能定位到部署、schema、Lua 文件或候选输出。
- 已记录真机手动验收覆盖 `rq` / `sj` / `xq` / `dt`；`ts` / `R123` / `cC1+1` / Unicode / UUID / 农历可在发布前继续补充验收记录。
...
- 文档与实际 UI/行为一致。
- 发布前验收矩阵记录 Lua smoke 的已验证范围和剩余补证据项。
