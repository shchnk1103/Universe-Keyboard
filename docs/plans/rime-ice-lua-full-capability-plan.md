...
- 已在雾凇拼音详情页增加 `高级输入功能` 状态区，展示基础检查、未安装、未使用、需要重新应用、暂不可用等普通用户文案。
- 已提供 `设为当前方案`、`重新应用 RIME 设置`、`重新下载雾凇拼音` 和 `查看诊断日志` 等恢复入口。
...
- `需要重新应用`：设置或 schema 文件变化后，还没有重新部署。
...
- 开关开启时，恢复完整雾凇 Lua 组件。
- 切换后标记 `rime_needs_deploy = true`，并引导用户重新应用 RIME 设置，或沿用现有自动部署模式。
...
- 在键盘中切换到雾凇拼音。
- 输入 `rq`，候选栏出现当天日期。
- 关闭高级输入功能并重新应用，`rq` 不再出现日期候选，但基础拼音仍可输入。
- 重新开启后恢复。
