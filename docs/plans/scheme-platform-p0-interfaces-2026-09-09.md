...
- `2026-09-10 Asia/Shanghai`: Human finalized **Section B try-failure state machine** — Decided pointer in §2; `idle`→`trying`→(`succeeded_pending_confirm`|`failed`)→`idle`; snapshot; no binding until confirm; deps pre-block; fail→immediate restore stay B; success+confirm→promote A; hide-from-B manual only; B never uninstall auto-rebind; P1 may add rollback try-deploy seam. Full rules in [`scheme-platform-discovery-layout-ux-2026-09-09.md`](scheme-platform-discovery-layout-ux-2026-09-09.md). Local commit only; no push.
