...
| P2 | **Landed locally (`2026-09-12`):** Migrate Wanxiang onto platform (lua / layout / **SharedDefault → `privatePreset`** same as Ice — `wanxiang_preset.yaml`; **Ownership stays dual** — Wanxiang platform path **keeps `exactHash`**); plan/post `wanxiang-plan-2`/`wanxiang-post-2`; staged SHA recomputed; `consumePrelude` **not** Wanxiang end-state; fidelity risk → **product regression after push**; **ask before push**; keep Active |
