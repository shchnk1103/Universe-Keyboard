...
| P1 | Extract Ice hooks → platform; Ice behavior unchanged. **Human-approved order (`2026-09-10`):** P1-0→P1-6 — see [`scheme-platform-p1-extract-plan-2026-09-10.md`](scheme-platform-p1-extract-plan-2026-09-10.md). **P1-0 (done):** plan + Ice regression checklist (docs). **P1-1 (pushed on #102 / CI green `a092de8`):** SchemeAdapter registry + Ice/Wanxiang/Luna adapters + equality tests；thin postProcessingRevision bridge. **P1-2 (done locally this tip):** LayoutCapability + route `isNineKeyCapable`/`isTwentySixKeyCapable` via registry；Ice T9 unchanged；Wanxiang nine=false. **P1-3…P1-5:** remaining Swift slices. **P1-6:** summary + IQ (no silent conditional close). **KOS:** local commit per slice; ask before push; no ADR Accept. Scope: LayoutCapability + ResourceCapability/ownership; adapter lookup (Ice-only nine-key; Wanxiang `supportsNineKey=false`); SharedDefault = Ice `privatePreset` only; Ownership = `namedList`+`exactHash`. **Not P1 UI:** Discovery / layout-picker |
...
- `2026-09-11 Asia/Shanghai`: P1-2 LayoutCapability landed locally (nineKeySchemaIDs; RimeRuntimeSelection/SchemaManager capability guards via SchemeAdapter registry; Ice T9 unchanged; Wanxiang nine=false); base tip a092de8 P1-1 CI green; local commit only; ask before push; next P1-3 awaiting auth.
