...
> **Run ID:** `RELEASE-2026-0801-04-B7-P2-20260824`
> **Status:** `Invalidated — Product Lead authorized device reboot after Extension failed to naturally leave pre-action baseline`
...
> **Predecessor:** [`P1 invalidated manifest`](release-2026-08-01-04-build7-device-run-2026-08-24.md)

## Scope、authority 与 non-claims

Human Product Owner 已授权继续完成冻结 RC 的真机工作，并担任本轮 iPhone Device Operator；随后于
`2026-08-24 Asia/Shanghai` 明确授权有界证据例外、额外只读 schema 预检，以及在 P1 失配后切换至
`rime_ice` 并冻结新 manifest。本轮只观察已安装的 Build 7 ad hoc Release 包；不构建、不重新安装、不改源码、
不上传 TestFlight，也不接受失败或跳过的发布风险。

P1 因实际为万象拼音而失效后，Device Operator 在授权范围内将当前方案切换为雾凇拼音；页面刷新后显示
“当前使用”和“基础检查通过”，未额外重新部署。键盘布局页面显示九宫格拼音已选中，输入方案为雾凇九键
（T9）。多个方案仍保持已安装是保留本地资源缓存的正常状态，不属于本轮缺陷或配置失配。

Final pre-action 查询发现 Main App 与 Keyboard Extension 均在运行；关闭 Main App 并等待后，Extension 仍以同一
PID 驻留。Human Product Lead 随后明确授权重启 iPhone，以取得冷进程基线并保留正式 C 阶段唯一一次 SIGTERM。
重启改变 boot session，因此本 P2 manifest 永久失效；不得在重启后沿用。失效前未关闭 Full Access、未打开
正式宿主字段、未输入，formal Human round 保持 `0`。
...
| Schema/layout | `rime_ice` / 雾凇拼音 `nightly` 显示“当前使用”且“基础检查通过”；主键盘为九宫格拼音，九宫格输入方案为雾凇九键（T9）。26-key English 仍按产品设计在九键偏好下使用 26 键；正式中文输入以 9-key `64` path/候选/删除 smoke 为主。高级输入功能页面只承载 Lua 增强偏好，不作为九宫格证据，本轮未改其开关。CoreDevice 在 iOS 27 上仍无法读取 App Group 根级 `Rime/shared`，因此按 bounded exception 保留为 Device-attested 配置边界 |
...
| P2 installed receipt refresh | Universe Keyboard `1.0 (7)` | `raw/p2-freeze-installed-app.json`, SHA-256 `1abcac04cf1d0409004b355cb3b927b84525f497cd07144785a02f0149af06b5` |
| P2 device/OS refresh | 同一 physical iPhone 13 Pro / iOS `27.0 (24A5418b)`；wired / paired / connected / booted；Developer Mode enabled | `raw/p2-freeze-device-details.json`, SHA-256 `84343e3937654220274dfe786904ecf59a4452427922fe6e20507bb9ca656045` |
| P2 current schema | `Device-attested`: 雾凇拼音 `nightly` 为“当前使用”，页面显示“基础检查通过” | `raw/read-only-preflight-rime-ice-current.png`, SHA-256 `c069c32b5f933033631234b97179cec71493bcdcaf1d6f4a09342269e4bc2819`, `382,206` bytes；设为当前方案后页面一度保留陈旧“需要重新部署”状态，退出重进即显示基础检查通过，说明既有部署已生效，因此没有执行冗余重新部署；不得将此扩大为机器 config digest |
| P2 layout/T9 | `Device-attested`: 九宫格拼音已选中；雾凇九键（T9）已选中 | `raw/read-only-preflight-keyboard-layout-t9.png`, SHA-256 `57efafc7e6143ac1a7ca896fbfb146b4385aa431d8cd58802315de46db5b175e`, `267,557` bytes |
| Advanced-input boundary | 该页面描述日期、时间、计算器等 Lua 增强功能；不承载主键盘布局选择，本轮未改开关 | `raw/read-only-preflight-advanced-input.png`, SHA-256 `bbf54cbcb7a3289c76e59dad7c938eed9622734c015d4613a95db93a26754e2b`, `359,502` bytes |
| Final pre-action identity refresh | receipt 仍为 `1.0 (7)`；同一 physical iPhone 13 Pro / iOS `27.0 (24A5418b)` | installed-app JSON SHA-256 `623f1edfee88d91be021b8e55482e4dc520aac62b250ccceb164d3f4be0a50e6`；device-details JSON SHA-256 `efbf8c8fa0add9a42bc232cfa747214a35f1c99814e8b03dbf1905b878e336ad` |
| Final pre-action crash/Jetsam refresh | Keyboard `11`、JetsamEvent `62`，与冻结前基线数量相同 | crash JSON SHA-256 `315227466bc462bd7e2e21cc45bf1ab5cb70a0d14a1791938abd56ad5f89e302`；Jetsam JSON SHA-256 `f406d3af304e71c341670c24ade800e286c0ea3e560276d017e16d94d928b49b` |
| Final pre-action process query | Hold：`2` 行，分别为当前 Main App 与 Keyboard Extension；这是设置页面使用后的合理运行态，但不满足 cold Extension 的零进程前置条件 | `raw/formal-pre-run-keyboard-processes.json`, SHA-256 `64f53bff01f15e0b1201d215950e9ca80542127fee5790bd234e163b67899b00`；不得提前用 C 阶段保留的精确终止动作清理 |
| Natural cleanup attempts | Device Operator 从 App 切换器关闭 Main App 后，查询只剩唯一 Keyboard Extension PID；再等待 `20 s` 后同一 PID 仍驻留 | after-close JSON SHA-256 `be830b22ee10b5fcf2a96558b5606673930675d6b662200f5b22d27f6a1e4e43`；after-wait JSON SHA-256 `49dbac083503512a70bc902691f24f6faae0429abde642853f107d6de9190e79`；未发送 signal、未输入、formal Human round 仍为 `0` |
...
```json
{
  "runId": "RELEASE-2026-0801-04-B7-P2-20260824",
  "templateState": "invalidated-authorized-device-reboot",
  ...
  "schemaConfigDeviceAttested": true,
  "machineConfigDigestAvailable": false,
  "extraReadOnlyPreflightUsed": 1,
  "preflightSessionCount": 1,
  "preflightObservationArtifactCount": 3,
  "humanRoundsUsed": 0,
