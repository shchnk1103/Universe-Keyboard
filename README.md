...
✅ **雾凇拼音 + librime-lua 集成完成 + 企业级重构 + 性能优化 + 闪烁修复。** 键盘支持完整 Lua 脚本功能，225 个单元测试，0 失败。按键音播放移至后台队列。RIME session 支持断线自动恢复。展开候选面板高度限制防止键盘跳变。view.alpha=0 + 最终高度触发显示缓解 iOS 3 阶段 resize 闪烁。统一日志系统支持按分类过滤（性能/画面/引擎/配置/部署/通用）。
...
│
└── Packages/
    ├── KeyboardCore/            # 纯逻辑层 (SPM, 21 个源文件, 225 个单元测试)
