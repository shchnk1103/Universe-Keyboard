...
AI/开发者应先阅读 `AGENTS.md`，然后从 `docs/KNOWLEDGE_INDEX.md` 按任务导航。文档权威来源、ADR、计划归档与 review 规则见 `docs/DOCUMENTATION_GOVERNANCE.md`。

## 当前状态

✅ **Swift 6 严格并发迁移 + 统一 RIME 桥接边界 + 候选栏稳定性修复。** App 与 Keyboard Extension 以 Swift 6 构建；RIME 会话和主 App 部署能力统一由 `Packages/RimeBridge` 提供。完整部署只允许在主 App 中完成，切换到键盘后输入路径不得触发部署。测试数量会持续变化，以本页的验证命令和当前 CI 结果为准。
...
- **RIME 候选学习与用户词典备份** — 按方案管理学习开关、本地备份、恢复和重置
...
│   │   ├── Settings/           # RIME、反馈、外观、高级输入与纠错设置
│   │   ├── Diagnostics/        # DiagnosticsView
│   │   ├── Dictionary/         # 本地词典浏览与索引
│   │   └── License/            # LicenseView
│   └── Services/               # SchemaManager 编排、安装、部署与用户词典备份
│
├── Keyboard/                   # 键盘扩展 (UIKit + ObjC)
│   ├── Controllers/            # KeyboardViewController + 按职责拆分的扩展/协调器
│   ├── Views/
│   │   ├── KeyPopupView.swift
│   │   ├── CandidateBar/       # collection view、cell、data source 与滚动样式
│   │   └── EmojiDataSource.swift
│   ├── Services/               # KeyClickPlayer, UITextDocumentProxyAdapter
│   └── Bridge/                 # KeyboardType+UIKit
│
└── Packages/
    ├── KeyboardCore/            # 纯逻辑层 (SPM，测试数以 swift test 输出为准)
...
# 先用 `xcrun simctl list devices available` 选择本机已安装的 simulator。
...
| `RimeBridgeTests` | iOS Simulator 契约测试；数量以当前 test 输出为准 |
| `UniverseKeyboardTests` / `KeyboardTests` | iOS Simulator 的 store/model/coordination 契约测试；数量以当前 test 输出为准 |
