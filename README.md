...
- **横向滚动候选栏** — UIScrollView 驱动、可视区高亮（滚出视口的候选自动变暗）、右侧 SF Symbol 展开按钮弹出多行候选面板替代键盘区域、渐隐遮罩，贴近原生体验。候选数据模型使用 `CandidateKind` 枚举 + `CandidateItem` 结构体，编译期类型安全。通过 `UIButton.tag`（非 accessibilityIdentifier）传递按钮类型
...
└── Packages/
    └── KeyboardCore/            # 纯逻辑层 (Swift Package)
        ...
        │   ├── KeyboardState.swift        # 键盘状态 + 所有子枚举
        │   ├── KeyboardAction.swift       # 用户动作枚举
        │   ├── KeyboardEffect.swift       # UI 刷新标记
        │   ├── CandidateItem.swift        # 候选数据模型 (CandidateKind 枚举 + CandidateItem 结构体)
        │   ├── CandidateProvider.swift    # 候选词协议
        │   ├── TextInputClient.swift      # 文本输入抽象
        │   └── FakeCandidateProvider.swift # 测试用假候选实现
