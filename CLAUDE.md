...
- **Candidate bar** uses `UIScrollView` for horizontal scrolling (`decelerationRate: .fast`). A `CAGradientLayer` fade mask on the scroll view fades the right edge. A fixed SF Symbol `chevron.down`/`chevron.up` button at the right edge toggles a multi-row candidate panel; the chevron rotates 180° with `curveEaseInOut` animation. The expanded panel **replaces** the keyboard rows (via `reloadKeyboardContent()`), occupying the full keyboard area with a filler view. Selecting a candidate or tapping the chevron again restores the keyboard with a `transitionCrossDissolve` animation. Keyboard page state is preserved across expand/collapse since `isCandidateExpanded` flag is toggled without changing `controller.state.currentPage`. Candidate buttons use `UIButton(type: .system)` with `setTitle(_:for:)` for reliable intrinsic sizing. Composition in secondary color, first candidate in tint color, placeholders non-interactive in tertiary color. Currently uses hardcoded fake pinyin candidates — RIME/librime integration has not started yet.
