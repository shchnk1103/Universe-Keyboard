...
- **Candidate bar** uses `UIScrollView` for horizontal scrolling with native-feel deceleration (`decelerationRate: .fast`). A `CAGradientLayer` fade mask on the right edge indicates overflow content. Candidate buttons auto-size to content width (no equal-width distribution). Composition string appears in secondary color, first candidate highlighted in tint color and semibold. Placeholder candidates are non-interactive. Currently uses hardcoded fake pinyin candidates — RIME/librime integration has not started yet.
