...
- **Candidate bar** uses `UIScrollView` for horizontal scrolling with native-feel deceleration (`decelerationRate: .fast`). A `CAGradientLayer` fade mask on the scroll view (not the container) fades the right edge of scrollable content. A fixed "…" expand button at the right edge opens a multi-row candidate panel (4-column grid) inserted into the root stack; selecting a candidate or tapping "收起" dismisses it with animation. Candidate buttons use `UIButton.Configuration` (iOS 15+), with `baseForegroundColor` for tint and `titleTextAttributesTransformer` for font. Composition string appears in secondary color, first candidate highlighted in tint color. Placeholder candidates are non-interactive with tertiary color. Currently uses hardcoded fake pinyin candidates — RIME/librime integration has not started yet.
