...
- **Candidate bar** uses `UIScrollView` for horizontal scrolling (`decelerationRate: .fast`). A `CAGradientLayer` fade mask on the scroll view fades the right edge. **Visibility-based highlighting**: `UIScrollViewDelegate.scrollViewDidScroll` computes each candidate button's intersection ratio with the visible bounds; alpha interpolates smoothly from 0.35 (scrolled out) to 1.0 (fully visible), creating a native-feel focus effect. A fixed SF Symbol `chevron.down`/`chevron.up` button at the right edge toggles a multi-row candidate panel; the chevron rotates 180° with `curveEaseInOut` animation. The expanded panel **replaces** the keyboard rows (via `reloadKeyboardContent()`), occupying the full keyboard area with a filler view. Selecting a candidate or tapping the chevron again restores the keyboard with `transitionCrossDissolve` animation. Keyboard page state is preserved: `isCandidateExpanded` flag is toggled without changing `controller.state.currentPage`. Candidate buttons use `UIButton.Configuration.plain()` with `NSDirectionalEdgeInsets` content insets and `AttributedString` for font/color (no deprecated `contentEdgeInsets`). Composition in secondary color, first candidate in tint color, placeholders non-interactive in tertiary color. Currently uses hardcoded fake pinyin candidates — RIME/librime integration has not started yet.
